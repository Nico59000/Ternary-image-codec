// ============================================================================
//  TERNARY IMAGE CODEC — Balanced Ternary 27‑Trit Superframe (Single Header)
//  Version: v6 (full, documented)
//
//  WHAT'S IN THIS FILE
//  -------------------
//  • SECTION 1  — Base Types & Constants
//  • SECTION 2  — Profiles, UEP, Interleaving, Scrambler, Beacon, Coset Pilot
//  • SECTION 3  — Superframe Header + Real Ternary CRC‑12
//  • SECTION 4  — GF(3^3) Arithmetic (p(x)=x^3+2x+1) & RS(26,k) Codec
//  • SECTION 5  — 27‑Trit Words & RAW Packing (2 pixels ↔ 1 word)
//  • SECTION 6  — 2D Interleaving (Boustrophedon) — Encode & Decode
//  • SECTION 7  — Subword Encapsulation (S27/S24/S21/S18/S15) & Centering
//  • SECTION 8  — Pipeline Contexts & High‑Level APIs (Profiled ↔ RAW)
//  • SECTION 9  — Self‑Tests (RS & API Round‑Trip)
//
//
//  License: MIT 
// ============================================================================
...
/// Balanced trit for in‑RAM logic (−1,0,+1). Not used on the wire.
enum class BTrit : int8_t { M1=-1, Z0=0, P1=+1 };

/// Transport trit (unbalanced) ∈ {0,1,2}
using UTrit = uint8_t;

/// GF(27) field element index ∈ {0..26}
using GF27  = uint8_t;

static constexpr int TRITS_PER_WORD = 27;     ///< Trits per word
static constexpr int SYM_PER_WORD   = 9;      ///< GF(27) symbols per word (3 trits/symbol)
static constexpr int NUM_BANDS      = 9;      ///< 9 slots → 9 bands

/// Pack 3 trits → one GF(27) symbol (base‑3 little‑endian)
inline GF27 pack3_utrits_to_gf27(UTrit t0, UTrit t1, UTrit t2)
{
    return static_cast<GF27>(t0 + 3*t1 + 9*t2);
}

/// Unpack one GF(27) symbol → 3 trits (base‑3 little‑endian)
...
// -- 2.1 Profiles & RS parameters ------------------------------------------------
struct RSParams {
    uint8_t n=26, k=22;
};

enum class ProfileID : uint8_t {
    RAW_MODE        = 0xFF, // No ECC; RAW IA bridge
    P1_RS26_24      = 0,    // t=1
    P2_RS26_22      = 1,    // t=2 (default)
    P3_RS26_20      = 2,    // t=3
    P4_RS26_18      = 3,    // t=4
    P5_RS26_22_2D   = 4     // P2 + 2D interleaving
};
...
/// Uniform UEP (same RS on all 9 bands). Default: P2
...
{
    for(int b=0; b<NUM_BANDS; ++b) u.band_profile[b]=1; // P2
    u.band_profile[0]=2;
    u.band_profile[3]=2;
    u.band_profile[6]=2; // P3
}

// -- 2.3 Interleaving helpers -----------------------------------------------------
struct Tile2D {
    uint16_t w=0, h=0;
};               ///< Active when profile=P5
struct BandMap {
    std::array<uint8_t,NUM_BANDS> word_sym_pos_to_band{0,1,2,3,4,5,6,7,8};
};

// -- 2.4 Scrambler (additive mod‑3 per trit, applied symbol‑wise) -----------------
...
// -- 2.6 Coset pilot (metadata only; NEVER local parity) -------------------------
enum class CosetID : uint8_t { C0=0, C1=1, C2=2 };

// ============================================================================
//  SECTION 3 — SUPERFRAME HEADER & TERNARY CRC‑12
// ============================================================================

// -- 3.1 Subword modes (encapsulation of smaller payload per 27‑trit word) -------
...
{
    auto base = std_res_for(SubwordMode::S27);
    auto tx   = std_res_for(m);
    uint32_t x0 = (base.w - tx.w)/2;
    uint32_t y0 = (base.h - tx.h)/2;
    return {x0,y0,tx.w,tx.h};
}
...
struct SuperframeHeader {
    ...
    Tile2D         tile{};            // for P5
    ScramblerSeed  seed{};
    uint32_t       band_map_hash = 0; // reserved for custom band maps
    uint32_t       frame_seq     = 0;
    uint32_t       reserved      = 0;
    uint32_t       crc3m         = 0; // 12‑trit CRC remainder (packed in 4 symbols)
    SparseBeaconCfg beacon{};         // optional sparse beacon
    // NEW (v6):
    SubwordMode    subword = SubwordMode::S27; // S27/S24/S21/S18/S15
    bool           centered = true;            // image centered in the 8K canvas
    CosetID        coset    = CosetID::C0;     // pilot coset (metadata only)
};
...
// -- 3.3 CRC‑12 ternary (g(x) = x^12 + x^7 + x^4 + x^3 + 1) ----------------------
struct CRC3 {
    static constexpr int L = 12;
    static inline void compute_remainder_12trits(const std::vector<UTrit>& msg,
            std::array<UTrit,L>& rem)
    {
        std::array<UTrit,L> reg{};
        reg.fill(0);
        auto step = [&](UTrit in) {
...
// -- 3.4 Header pack/unpack with CRC in symbols[20],[21],[22],[26] ---------------
struct HeaderCodec {
    static HeaderPack pack(const SuperframeHeader& h)
    {
        HeaderPack p{};
        auto at=[&](int i, GF27 v) {
            p.symbols[i]=static_cast<GF27>(v%27);
        };
        // [0..3] magic/version/profile
        at(0,h.magic%27);
        at(1,(h.magic/27)%27);
        at(2,h.version%27);
        at(3, static_cast<GF27>(h.profile));
        // [4..6] UEP (3× base‑27 triplets)
        ...
        at(10,h.seed.b%27);
        at(11,h.seed.s0%27);
        // [12] subword + centered (sub in 0..4, centered in {0,1} mapped via +9)
        {
            auto code_of=[&](SubwordMode m)->uint8_t{ switch(m)
        {
        case SubwordMode::S27:
            return 0;
        case SubwordMode::S24:
        ...
        uint8_t sub=code_of(h.subword);
            uint8_t cen = h.centered?1:0;
            at(12, static_cast<GF27>((sub + 9*cen) % 27));
        }
        // [13..16] band_map_hash (3 symbols) + coset (1 symbol)
        at(13,h.band_map_hash%27);
        at(14,(h.band_map_hash/27)%27);
        at(15,(h.band_map_hash/729)%27);
        at(16, static_cast<GF27>(static_cast<uint8_t>(h.coset)%3));
        // [17..19] frame_seq
        at(17,h.frame_seq%27);
        at(18,(h.frame_seq/27)%27);
        at(19,(h.frame_seq/729)%27);
        // [20..22] CRC placeholders; [23..25] beacon; [26] CRC tail
        at(20,0);
        at(21,0);
        at(22,0);
    ...
    static SuperframeHeader unpack(const HeaderPack& p)
    {
        ...
        // [12] subword + centered
        {
            ...
            uint8_t cen=(v/9)%3;
            uint8_t sub=v%9;
            auto mode_of=[&](uint8_t c)->SubwordMode{ switch(c)
        {
        case 0:
        ...
        h.subword = mode_of(sub);
            h.centered = (cen!=0);
        }
        h.band_map_hash = rd(13) + 27*rd(14) + 729*rd(15);
        h.coset = static_cast<CosetID>(rd(16)%3);
...
// ============================================================================
//  SECTION 4 — GF(27) ARITHMETIC & RS(26,k) CODEC
// ============================================================================

// -- 4.1 Basic GF(27) ops (poly mult modulo p(x)=x^3+2x+1) ----------------------
...
struct GF27Context {
    ...
    {
        return gf27_add_u(a,b);
    }
    inline GF27 sub(GF27 a,GF27 b) const
    {
        return gf27_sub_u(a,b);
    }
...
// -- 4.3 Reed–Solomon RS(26,k) (BM + Chien + Forney) -----------------------------
struct RSCodec {
    GF27Context* gf=nullptr;
    RSParams params{};
    std::vector<GF27> g; // generator poly (low→high)
    void init(GF27Context* ctx, RSParams p)
    {
        gf=ctx;
        params=p;
        build_generator();
...
inline void pack_two_pixels_into_word27(const PixelYCbCrQuant& p0,const PixelYCbCrQuant& p1,Word27& w)
{
    ...
    T[26]=0;
    for(int s=0; s<9; ++s) w.sym[s]=pack3_utrits_to_gf27(T[s*3+0],T[s*3+1],T[s*3+2]);
}
inline void unpack_word27_to_two_pixels(const Word27& w,PixelYCbCrQuant& p0,PixelYCbCrQuant& p1)
{
    std::array<UTrit,27> T{};
    for(int s=0; s<9; ++s) {
        auto d=unpack_gf27_to3_utrits(w.sym[s]);
        T[s*3]=d[0];
        T[s*3+1]=d[1];
        T[s*3+2]=d[2];
    }
    p0.Yq=ternary_digits_to_int(T,5,0);
    p0.Cbq=(int16_t)ternary_digits_to_int(T,4,5)-40;
    p0.Crq=(int16_t)ternary_digits_to_int(T,4,9)-40;
    p1.Yq=ternary_digits_to_int(T,5,13);
    p1.Cbq=(int16_t)ternary_digits_to_int(T,4,18)-40;
    p1.Crq=(int16_t)ternary_digits_to_int(T,4,22)-40;
}
...
inline bool encode_raw_pixels_to_words(const std::vector<PixelYCbCrQuant>& px,std::vector<Word27>& out)
{
    ...
    for(size_t i=0; i<px.size(); i+=2) {
        Word27 w{};
        pack_two_pixels_into_word27(px[i], (i+1<px.size()?px[i+1]:PixelYCbCrQuant{}), w);
        out.push_back(w);
    }
    return true;
}
inline bool decode_raw_words_to_pixels(const std::vector<Word27>& in,std::vector<PixelYCbCrQuant>& out)
{
    ...
    for(const auto& w: in) {
        PixelYCbCrQuant a{},b{};
        unpack_word27_to_two_pixels(w,a,b);
        out.push_back(a);
        out.push_back(b);
    }
    return true;
...
// ============================================================================
//  SECTION 7 — SUBWORD ENCAPSULATION & CENTERING (S27/S24/S21/S18/S15)
// ============================================================================

// -- 7.1 Subword extraction/injection (per word) ---------------------------------
inline void extract_subword_trits_from_word(const Word27& w,int payloadN,std::array<UTrit,27>& out)
{
    for(int s=0; s<9; ++s) {
        auto d=unpack_gf27_to3_utrits(w.sym[s]);
        out[s*3+0]=d[0];
    ...
    (void)payloadN; // caller uses first N entries
}
inline void inject_subword_trits_into_word(const UTrit* inN,int payloadN,Word27& w,UTrit side_fill=0)
{
    std::array<UTrit,27> T{};
    for(int i=0; i<payloadN; ++i) T[i]=inN[i];
    for(int i=payloadN; i<27; ++i) T[i]=side_fill;
    for(int s=0; s<9; ++s) w.sym[s]=pack3_utrits_to_gf27(T[s*3+0],T[s*3+1],T[s*3+2]);
}

// -- 7.2 Subword stream helpers (whole sequence) ---------------------------------
inline void extract_subword_stream_from_words(const std::vector<Word27>& words,int payloadN,std::vector<UTrit>& out)
{
    out.clear();
    out.reserve(words.size()*payloadN);
    std::array<UTrit,27> T{};
    for(const auto& w: words) {
        extract_subword_trits_from_word(w,payloadN,T);
        for(int i=0; i<payloadN; ++i) out.push_back(T[i]);
    }
}
inline void build_words_from_subword_stream(const std::vector<UTrit>& in,int payloadN,std::vector<Word27>& out,UTrit side_fill=0)
{
    out.clear();
    size_t idx=0;
    while(idx<in.size()) {
        Word27 w{};
        UTrit buf[27] {};
        int take=std::min(payloadN,(int)(in.size()-idx));
        for(int i=0; i<take; ++i) buf[i]=in[idx+i];
        inject_subword_trits_into_word(buf,payloadN,w,side_fill);
        out.push_back(w);
        idx+=take;
    }
}

// -- 7.3 Centering helpers -------------------------------------------------------
struct ActiveWindowWords {
    uint32_t x0w, y0, ww, hh;
};
inline ActiveWindowWords to_word_space(const ActiveWindow& win_px)
{
    ActiveWindowWords W{};
    W.x0w=win_px.x0/2;
    W.y0=win_px.y0;
...
struct EncoderConfig {
    ...
    // NEW (v6): subword metadata + placement
...
struct DecoderConfigSeen {
    ...
    ScramblerSeed  seed{1,1,1};
    SparseBeaconCfg beacon{};
    // Metadata (not mandatory at receiver side)
    SubwordMode    subword = SubwordMode::S27;
    bool           centered = true;
    CosetID        coset    = CosetID::C0;
};
...
// -- 8.2 Header IO (read/encode) -------------------------------------------------
inline bool read_and_decode_header_from_words(const std::vector<Word27>& words,
        size_t& cursor_word,
        SuperframeHeader& out_hdr,
        RSCodec& rs_hdr)
{
    if(cursor_word + 6 > words.size()) return false; // 6 words = 54 symbols (we use 52)
    std::vector<GF27> syms;
    syms.reserve(6*9);
    for(int w=0; w<6; ++w) for(int s=0; s<9; ++s) syms.push_back(words[cursor_word+w].sym[s]);
    cursor_word += 6;
    ...
    if(!HeaderCodec::check_crc(hp)) return false;
        ...
        const SuperframeHeader& hdr)
{
    ...
    if(hdr.beacon.enabled && hdr.beacon.words_period>0) {
        ...
        while(idx<sy.size()) {
            bool isb=((word_idx % hdr.beacon.words_period)==0);
            for(int slot=0; slot<9 && idx<sy.size(); ++slot, ++idx) {
                if(isb && slot==hdr.beacon.band_slot) { /* drop */ }
    ...
    size_t k=0;
    for(size_t w=0; w<nw; ++w) for(int s=0; s<9; ++s) words[w].sym[s]=sy[k++];
}
...
// -- 8.3 High‑level API: Profiled → RAW -----------------------------------------
                                  ...
                                  DecoderContext& dctx)
{
    ...
    if(dctx.cfg_last_seen.profile == ProfileID::RAW_MODE) {
        out_raw_words=in_profile_words;
        return true;
    }
    size_t cursor=0;
    ...
    if(!demap_and_rsdecode_bands_from_words(body, useful_syms, hdr, dctx.rs_p1, dctx.rs_p2, dctx.rs_p3, dctx.rs_p4)) return false;
    if(hdr.profile == ProfileID::P5_RS26_22_2D && hdr.tile.w && hdr.tile.h) {
        deinterleave2D_boustrophedon(useful_syms, hdr.tile);
    }
    // Symbols → trits → pack 26‑useful‑trit RAW words
    std::vector<UTrit> trits;
    trits.reserve(useful_syms.size()*3);
    for(auto s: useful_syms) {
        auto d=unpack_gf27_to3_utrits(s);
        trits.insert(trits.end(), d.begin(), d.end());
    }
    size_t idx=0;
    while(idx + 26 <= trits.size()) {
        std::array<UTrit,27> T{};
        for(int i=0; i<26; ++i) T[i]=trits[idx+i];
        T[26]=0;
        Word27 w{};
        for(int s=0; s<9; ++s) w.sym[s]=pack3_utrits_to_gf27(T[s*3],T[s*3+1],T[s*3+2]);
        out_raw_words.push_back(w);
        idx += 26;
    }
    return true;
}

// -- 8.4 High‑level API: RAW → Profiled -----------------------------------------
                                    ...
                                    EncoderContext& ectx)
{
    ...
    if(ectx.cfg.profile == ProfileID::RAW_MODE) {
        out_profile_words=in_raw_words;
        return true;
    }
    // RAW → useful symbols (group 26 useful trits into symbols by 3)
    std::vector<GF27> syms_useful;
    syms_useful.reserve(in_raw_words.size()*8);
    ...
    for(const auto& w: in_raw_words) {
        ...
        if(clen>0) {
            ...
            if(clen==3) {
                syms_useful.push_back(pack3_utrits_to_gf27(carry[0],carry[1],carry[2]));
                clen=0;
            }
        }
...
for(int b=0; b<9; ++b) {
    ...
    if(ectx.cfg.beacon.enabled && ectx.cfg.beacon.words_period>0) {
        std::vector<GF27> sy2;
        sy2.reserve(body_syms.size() + body_syms.size()/(9*ectx.cfg.beacon.words_period) + 9);
    ...
    size_t idx=0;
    for(size_t w=0; w<total_words; ++w) for(int s=0; s<9; ++s) out_profile_words[w].sym[s]=(idx<all.size()? all[idx++] : 0);
    return true;
}
...
inline bool selftest_rs_unit()
{
    ...
    for(ProfileID pid : {
        ...
        for(int e=0; e<t; ++e) {
            ...
            used.push_back(p);
            code[p]=gf.add(code[p], static_cast<GF27>(val(rng)));
        }
...
inline bool selftest_api_roundtrip(bool use_p5=false)
{
    ...
    EncoderContext ectx;
    if(use_p5) {
        ectx.cfg.profile=ProfileID::P5_RS26_22_2D;
        ectx.cfg.tile= {16,16};
    }
    else {
        ectx.cfg.profile=ProfileID::P2_RS26_22;
        ectx.cfg.tile= {0,0};
    }
    uep_luma_priority(ectx.cfg.uep);
